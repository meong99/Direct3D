...
void Texture::Load(const wstring& path)
{
	...
	HRESULT hr = ::CreateTexture(GEngine->GetDevice().Get(), _image.GetMetadata(), &_tex2D);
	if (FAILED(hr))
		assert(nullptr);

	vector<D3D12_SUBRESOURCE_DATA> subResources;

	hr = ::PrepareUpload(GEngine->GetDevice().Get(),
		_image.GetImages(),
	...
	uint64 tmp =  ::UpdateSubresources(GEngine->GetResCmdList().Get(),
		_tex2D.Get(),
	...
	GEngine->GetRenderController()->FlushResourceCommandQueue();
	...
	GEngine->GetDevice()->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&_srvHeap));
	...
	GEngine->GetDevice()->CreateShaderResourceView(_tex2D.Get(), &srvDesc, _srvHeapBegin);
}
	...
	D3D12_RESOURCE_FLAGS resFlags, Vec4 clearColor)
{
	...
	// Create Texture2D
	HRESULT hr = GEngine->GetDevice()->CreateCommittedResource(
		&heapProperty,
		heapFlags,
		&desc,
		resourceStates,
...
void Texture::CreateFromResource(ComPtr<ID3D12Resource> tex2D)
{
	...
	if (desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)
	{
		...
		heapDesc.NodeMask = 0;
		GEngine->GetDevice()->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&_dsvHeap));

		D3D12_CPU_DESCRIPTOR_HANDLE hDSVHandle = _dsvHeap->GetCPUDescriptorHandleForHeapStart();
		GEngine->GetDevice()->CreateDepthStencilView(_tex2D.Get(), nullptr, hDSVHandle);
	}
	else
	{
		if (desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET)
		{
			...
			GEngine->GetDevice()->CreateRenderTargetView(_tex2D.Get(), nullptr, rtvHeapBegin);
		}
